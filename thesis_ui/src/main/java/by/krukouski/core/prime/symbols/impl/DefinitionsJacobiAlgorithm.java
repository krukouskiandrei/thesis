...
@Component("definitionsJacobiAlgorithm")
public class DefinitionsJacobiAlgorithm implements Symbols {
	
private static Logger log = Logger.getLogger(DefinitionsJacobiAlgorithm.class);
	...
	//J(a, p) - The algorithm is based on the definitions and properties of the Jacobi symbol.
	public int calculate(BigInteger a, BigInteger p) throws SymbolsException {
		
		if(p.remainder(TWO).compareTo(BigInteger.ZERO) == 0){
			log.info("second number shouldn't be 0");
